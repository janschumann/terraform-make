...
MIN_SESSION_AGE ?= 15
AWS_SESSION_DURATION ?= 1hour

# aws session type (iam or sso)
AWS_SESSION_TYPE ?= iam
...
SESSION_EXPIRE = $(shell $(DATE_CMD) --utc --date "+$(AWS_SESSION_DURATION)" +"%s")
...
aws-debug: debug
	...
	@echo SESSION_EXPIRE=$(SESSION_EXPIRE)
...
show-account-id: verify-account-id
	@echo $(AWS_SESSION_CMD_IAM)
	@echo $(AWS_SESSION_CMD_SSO)
	@echo $(ACCOUNT_ID)
...
access-sso-$(AWS_ROLE_NAME):
	@if [ "1" = "$(IS_EXPIRED)" ]; then $(TERRAFORM_MAKE_LIB_HOME)/aws-session-sso.sh -p $(AWS_PROFILE) -e $(SESSION_EXPIRE); fi

access-iam-$(AWS_ROLE_NAME):
	@if [ "1" = "$(IS_EXPIRED)" ]; then $(TERRAFORM_MAKE_LIB_HOME)/aws-session-iam.sh -o $(ORGANISATION) -p $(AWS_PROFILE) -r $(AWS_ROLE_NAME); fi
